...
\usepackage{tablefootnote} % Para notas de rodapé dentro de tabelas

\usepackage{graphicx}
\usepackage{subcaption} % Para o ambiente subfigure

\usepackage{pdflscape} % Para orientação paisagem
\usepackage{booktabs}
\usepackage{siunitx} % Para alinhamento decimal (opcional)
\usepackage{xcolor}
\usepackage{colortbl}
\usepackage{graphicx} % Para \resizebox

\usepackage{hyperref}
% Defina a cor do destaque (pode ser ajustado)
% \definecolor{highlightcolor}{RGB}{255, 204, 203} % Um rosa claro
% Se a cor da sua imagem é mais para um vermelho/rosa suave, pode ser algo como:
\definecolor{highlightcolor}{HTML}{F8CECC} % Um tom de rosa/vermelho pálido

\begin{document}

% Escolha entre autor ou autora:
\autor{Bruno Henrique Nunes}
%\autora{Nome da Autora}

% Sempre deve haver um título em português:
\titulo{\textit{Framework End-to-End} com CNNs e \textit{Transformers} para Detecção de Câncer de Próstata em Patologia Digital}
...
% se necessário.
\datadadefesa{30}{5}{2025}

% Para a versão final defina:
% Repita o nome do Orientador(a) no primeiro avaliador
\avaliadorA{Profa. Dra. Ana Estela Antunes da Silva}{Faculdade de Tecnologia/UNICAMP}
\avaliadorB{Profa. Dra. Thaína A. Azevedo Tosta }{Instituto de Ciência e Tecnologia/UNIFESP}
\avaliadorC{Prof. Dr. Rangel Arthur}{Faculdade de Tecnologia/UNICAMP}
...
\begin{resumo} 
O câncer de próstata representa um desafio significativo para a saúde pública global, figurando entre as neoplasias mais incidentes em homens. O diagnóstico preciso, tradicionalmente baseado na análise histopatológica de biópsias digitalizadas (denominadas \Sigla{\textit{Whole Slide Images}}{WSI}), é fundamental para o prognóstico. Contudo, a análise manual de WSIs é demorada, complexa devido à dimensão das imagens e sujeita à variabilidade interobservador. Modelos de Aprendizado Profundo, como Redes Neurais Convolucionais (\Sigla{\textit{Convolutional Neural Networks}}{CNN}) e, mais recentemente, \textit{Transformers}, têm demonstrado potencial para auxiliar nessa tarefa. Entretanto, a literatura ainda carece de metodologias computacionais completas, eficientes e replicáveis. Esta dissertação aborda essa lacuna, propondo um \textit{framework} computacional \textit{end-to-end} para a segmentação semântica de tecido canceroso em WSIs. O método integra abordagens baseadas em CNNs e \textit{Transformers}, focando no estabelecimento de uma metodologia sistemática que abrange desde o pré-processamento eficiente das lâminas até a otimização de modelos híbridos. A validação utilizou métricas bem fundamentadas pela comunidade científica (Coeficiente de Dice, IoU, Acurácia) em bases de dados públicas (DIAGSET) e privadas. Os resultados demonstraram a eficácia do \textit{framework} na segmentação de câncer de próstata e sua capacidade de generalização para outras histopatologias, atingindo valores médios de Acurácia e Dice razoáveis em todos os cenários testados. 
\end{resumo}

% Sempre deve haver um abstract:
\begin{abstract}
Prostate cancer represents a significant global public health challenge, ranking among the most frequent neoplasms in men. Accurate diagnosis, traditionally based on histopathological analysis of digitized biopsies (known as \textit{Whole Slide Images} -- WSI), is crucial for prognosis. However, manual analysis of WSIs is time-consuming, complex due to image dimensions, and subject to inter-observer variability. Deep Learning models, such as Convolutional Neural Networks (CNNs) and, more recently, \textit{Transformers}, have shown potential to assist in this task. Nevertheless, the literature still lacks complete, efficient, and replicable computational methodologies. This dissertation addresses this gap by proposing an \textit{end-to-end} computational \textit{framework} for the semantic segmentation of cancerous tissue in WSIs. The method integrates CNN and \textit{Transformer}-based approaches, focusing on establishing a systematic methodology that encompasses everything from efficient slide pre-processing to the optimization of hybrid models. Validation employed metrics well-established by the scientific community (Dice Coefficient,IoU, Accuracy) on public (DIAGSET) and private datasets. The results demonstrated the efficacy of the \textit{framework} in prostate cancer segmentation and its capability to generalize to other histopathologies, achieving average Accuracy and Dice resonable in all tested scenarios.
\end{abstract}

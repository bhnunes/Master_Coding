...
\addbibresource{bibliografia_revisao_sistematica.bib}

% Pacotes adicionais usados pelo texto da dissertação.
\usepackage{array}
\usepackage{float}
...
% A classe oficial é preservada, mas a seleção explícita de fontes evita que
% o XeLaTeX substitua a Libertine por Computer Modern quando estilos T1 faltam.
...
\titulo{Fluxo de Trabalho Reprodutível para Detecção de Câncer em Imagens Histopatológicas de Lâminas Inteiras}
...
\avaliadorB{Prof. Dr. A definir}{Instituição a definir}
\avaliadorC{Prof. Dr. A definir}{Instituição a definir}
...
% que contém os resultados e a discussão.
\include{resultados}

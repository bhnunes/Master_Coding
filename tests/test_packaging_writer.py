...
def test_write_patch_dataset_hdf5_writes_source_dataset_contract(tmp_path: Path) -> None:
    image_path = tmp_path / "CANCER" / "PATIENT_1_a.png"
    mask_path = tmp_path / "CANCER_MASK" / "PATIENT_1_a.png"
    _write_rgb_png(image_path)
    _write_mask_png(mask_path)

    output_path = tmp_path / "SOURCE_DATASET.h5"
    write_patch_dataset_hdf5(
        output_path,
        [
            SampleRecord(
    ...
    with h5py.File(output_path, "r") as handle:
        ...
        source_image_paths = cast(h5py.Dataset, handle["source_image_paths"])
        source_mask_paths = cast(h5py.Dataset, handle["source_mask_paths"])
        ...
        assert cast(bytes, source_image_paths[0]).decode("utf-8") == str(image_path)
        assert cast(bytes, source_mask_paths[0]).decode("utf-8") == str(mask_path)
        assert set(np.unique(masks[0]).tolist()) <= {0, 1}


def test_write_patch_dataset_hdf5_reuses_existing_output_when_signature_matches(
    tmp_path: Path,
) -> None:
    image_path = tmp_path / "CANCER" / "PATIENT_1_a.png"
    mask_path = tmp_path / "CANCER_MASK" / "PATIENT_1_a.png"
...
def test_write_patch_dataset_hdf5_rejects_existing_output_when_signature_mismatches(
    tmp_path: Path,
) -> None:
    output_path = tmp_path / "SOURCE_DATASET.h5"
...
def test_write_patch_dataset_hdf5_rejects_reuse_when_image_contents_change_in_place(
    tmp_path: Path,
) -> None:
    image_path = tmp_path / "CANCER" / "PATIENT_1_a.png"
    mask_path = tmp_path / "CANCER_MASK" / "PATIENT_1_a.png"
    ...
    write_patch_dataset_hdf5(output_path, [sample], img_size=16, overwrite=True)
    ...
    with pytest.raises(ValueError, match="does not match the current split inputs"):
        write_patch_dataset_hdf5(output_path, [sample], img_size=16, overwrite=False)


def test_write_patch_dataset_hdf5_does_not_prehash_inputs_before_writing(
    tmp_path: Path,

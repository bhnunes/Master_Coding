...
def test_train_epoch_resolves_precision_once_per_epoch(monkeypatch: pytest.MonkeyPatch) -> None:
    calls: list[tuple[str, str]] = []

    def fake_setup_precision(
        architecture: str, amp_precision: str
    ) -> tuple[Any, None, dict[str, str]]:
        calls.append((architecture, amp_precision))
        return torch.float32, None, {"amp": "ok"}

    monkeypatch.setattr("helpers.training.loop.setup_precision", fake_setup_precision)
...
def test_validate_epoch_resolves_precision_once_per_epoch(

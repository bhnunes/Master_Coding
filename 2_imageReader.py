...
def process_window(args):
    ...
    Reads the patch, checks tissue %, checks annotation overlap %, saves patch and mask if criteria met.
    """
    ...
    try:
        # Open slide within the worker process - necessary as OpenSlide objects aren't easily pickled
        ...
        if patch_saved:
            ...
            file_basename = f"{label}_PATIENT_{patient}_{x_int}_{y_int}_{random_number}_{timestamp}.png" # Include coords

            # Save Image Patch
            save_path_img = os.path.join(save_folder_img, file_basename)
            patch_pil.save(save_path_img)

            # Save Corresponding Mask (0 for non-cancer, 255 for cancer for visualization)
            save_path_mask = os.path.join(save_folder_mask, file_basename)
            mask_image = Image.fromarray(final_mask_to_save * 255) # Scale mask to 0/255
        ...
        print(error_message, file=sys.stderr) # Optional: print worker errors
...
def extract_patches_for_slide(path_Image, target_level, window_size, stride,
    ...
    """
    Loads annotations, generates candidate windows, filters them, and processes them in parallel.
    """
    slide = None
    ...
    try:
        ...
        # --- Prepare arguments for parallel processing using ONLY filtered coordinates ---
        args_list = [(path_Image, target_level, window_size, stride,
                      tissue_percentage_req, match_percentage_req,
                      path_cancer_folder, path_not_cancer_folder,
                      path_cancer_mask_folder, path_not_cancer_mask_folder,
                      patient, x, y,
                      annotations_cancer_level0, annotations_not_cancer_level0) # Pass level 0 annos

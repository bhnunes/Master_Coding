...
\newcommand{\ArtifactPatchIndex}{\texttt{PATCHES/\allowbreak artifact\_\allowbreak patch\_\allowbreak index.parquet}}
...
\newcommand{\RuntimeVahadaneExact}{\texttt{RUNTIME\_\allowbreak VAHADANE\_\allowbreak BACKEND=\allowbreak torch\_\allowbreak staintools\_\allowbreak exact}}
\newcommand{\TrainSelectedCompactDir}{\texttt{TRAIN\_\allowbreak SELECTED\_\allowbreak COMPACT\_\allowbreak DIR}}
\newcommand{\SmartSamplerUseGist}{\texttt{SMART\_\allowbreak SAMPLER\_\allowbreak USE\_\allowbreak GIST}}
\newcommand{\SamplingDecision}{\texttt{sampling\_\allowbreak decision}}
\newcommand{\StageSevenSelected}{\texttt{is\_\allowbreak stage7\_\allowbreak selected}}
\newcommand{\LastHiddenState}{\texttt{last\_\allowbreak hidden\_\allowbreak state[:,0,:]}}
\newcommand{\RegistryNotNormalized}{\texttt{training\_\allowbreak model\_\allowbreak registry\_\allowbreak NOT\_\allowbreak NORMALIZED.json}}
...
\title[mode=title]{A Reproducibility-Oriented Workflow for Whole-Slide Cancer Detection in Digital Pathology}
...
\item Training and inference stages incorporate explicit reproducibility controls, runtime-normalization lineage, and compatibility checks to reduce stale-artifact reuse.
\end{highlights}
...
The repository is organized as a script-driven pipeline in which each numbered entrypoint represents a distinct processing stage. Stage~1 generates GeoJSON artifact annotations directly from archived whole-slide images. Stage~2 ingests cases into SQLite and extracts HDF5 patch shards from slide annotations. Stages~3.1--3.3 perform human-in-the-loop review preparation and graph-based cleaning of cancer patches, and the resulting accepted rows can be repackaged into a canonical source HDF5 before model development. Stage~4 creates one patient-level \texttt{TRAIN}, \texttt{VALIDATION}, and \texttt{TEST} split assignment, computes shared entropy and template-selection provenance, and persists runtime stain-normalization artifacts for downstream selection. Stage~5 performs scientific sanity checks over manifests, provenance, HDF5 contracts, and label--mask semantics. Stage~6 optionally filters training rows by embedding-based smart sampling and can publish compact selected-TRAIN storage. Stage~7 performs repeated learning-rate screening over an approved model registry. Stage~8 trains one segmentation model per execution, with optional artifact-aware loss and OHEM. Stage~9 optimizes a two-stream ensemble recipe using the validation split only, and Stage~10 evaluates the frozen recipe on the test split.

The pipeline is helper-first rather than package-first. Root scripts remain orchestration layers, whereas implementation details are delegated to helper modules grouped by domain. This organization is scientifically relevant because configuration loading, provenance capture, normalization, sampling, and evaluation behavior are not hidden in notebooks or ad hoc scripts, but encoded as explicit reusable modules. The current codebase also centralizes shared runtime behaviors, including headless plotting setup, native warning suppression, runtime-normalization status reporting, and manifest path resolution, so operational fixes apply consistently across stages.
...
Stage~2 no longer uses artifact estimates as a hard rejection criterion. Instead, when advanced artifact handling is enabled, it computes per-patch coverage fractions for each Stage~1 artifact class and writes them into \ArtifactPatchIndex. When the option is disabled, the same schema is preserved with zero-valued coverage fields. This design keeps the training contract stable while allowing artifact-aware learning to be enabled or disabled without changing downstream file interfaces.

The master manifest is treated as a durable data contract rather than a convenience index. Current read paths validate the SQLite database before consuming manifest-backed row state, and Stage~2 records source-HDF5 signatures so later cleaning and split-generation steps can fail closed when a stale artifact is applied to regenerated data. Operationally, native TIFF/OpenSlide metadata warnings can be suppressed at the runtime boundary to avoid log flooding from non-fatal slide metadata noise while preserving hard failures for unreadable slides or broken database state.
...
\begin{equation}
\tau^*(\theta)=\argmax_{\tau\in\mathcal{T}}
...
\subsection{Stage 4: patient-level split generation and shared runtime-normalization artifacts}

Stage~4 reads accepted canonical rows from the Stage~2 master manifest and constructs one patient-disjoint \texttt{TRAIN}, \texttt{VALIDATION}, and \texttt{TEST} split assignment. Patient stratification is based on the maximum patch label observed within each stored patient identifier, thereby preserving the patient as the unit of leakage prevention under the identifier assumption described above. Split search is randomized but constrained by configurable patient counts and split-ratio targets, and the resulting split is persisted once as a Stage~4 split bundle in SQLite rather than as one normalization-specific split per method.

The current Stage~4 design computes entropy on \texttt{TRAIN} once per run, selects one high-entropy template patch per training patient, and aggregates those templates into a shared target image. That shared target is then used to persist runtime-ready artifacts for \texttt{REINHARD}, \texttt{RUIFROK}, \texttt{MACENKO}, and \texttt{VAHADANE}, while \texttt{NOT\_NORMALIZED} is treated as the no-artifact runtime option. This train-only target-building policy is scientifically important because it prevents distributional information from validation or test data from leaking into preprocessing while avoiding redundant Stage~4 split generation across normalization methods.

The stage writes provenance artifacts including \texttt{manifest.csv}, \texttt{split\_stats.csv}, \texttt{run\_config.json}, template-selection sidecars, optional entropy-cache CSV files, and runtime-normalization state JSON files. Downstream stages resolve the selected normalization method at runtime from \RuntimeNormMethod{} and the Stage~4 split-bundle artifact registry recorded in \MasterManifest, rather than from per-patch persisted normalization state.
...
Stage~6 optionally reduces the effective training set while preserving manifest-backed provenance and validation/test semantics. The stage reads \texttt{TRAIN} rows from \MasterManifest, loads pixels from canonical Phase~2 patient shards, builds a patient-wise patch index, and extracts Phikon-v2 embeddings. Positive-label rows and rows with positive mask pixels are protected from reduction by default, so the diversity selector operates only on reducible candidates while protected rows remain eligible for downstream training. This policy makes the storage optimization explicit: selection changes the training-row subset, but it does not rewrite validation or test rows.

For a patch indexed by $i$, let $H_i\in\mathbb{R}^{T\times D}$ denote the final hidden-state matrix returned by the image model, where $T$ is the number of tokens and $D$ is the hidden dimension. The embedding used for smart sampling is the first-token descriptor
\[
z_i = H_i[0,:]\in\mathbb{R}^{D}.
\]
Here, $z_i$ is the embedding for patch $i$ and corresponds to the \LastHiddenState{} tensor returned by the model. For each patient, the stage starts from an initial candidate count $n_0$ and iteratively increases the embedding budget according to
\[
...
Once the patch budget for a patient is determined, the default diversity selector uses \texttt{MiniBatchKMeans}. Patches are then sampled approximately uniformly across clusters, and the implementation persists sidecar summaries plus SQLite row-state updates such as \SamplingDecision{} and \StageSevenSelected. When \SmartSamplerUseGist{} is enabled, the final reducible-candidate selector can instead use a GIST-style facility-location objective. For large patient pools, however, the implementation first constructs a bounded embedding-space landmark pool before running the GIST-style selector. That engineering constraint should be reported as a GIST-inspired or GIST-style large-pool approximation rather than as the paper-exact GIST algorithm, because the objective is no longer optimized over the full reducible candidate set.
...
To reduce downstream I/O cost under mounted-drive or small-local-disk constraints, Stage~6 can build a compact \texttt{TRAIN\_SELECTED} artifact after selection. This artifact contains HDF5 shards for selected training rows only, an \texttt{index.sqlite} mapping canonical source rows to compact rows, and a \texttt{summary.json} provenance record. The artifact is built in scratch space and then published to \TrainSelectedCompactDir. Stages~7 and~8 may remap selected \texttt{TRAIN} rows to this compact storage, whereas validation and test evaluation continue to use canonical Phase~2 patient shards.
...
For each sampled loss triplet $(\alpha,\beta,\gamma)$ and model pair, the stage performs repeated learning-rate-range tests using \texttt{AdamW}, configurable automatic mixed precision that defaults to \texttt{fp32}, class rebalancing, and the selected runtime-normalization method. If smart sampling and compact storage are enabled, \texttt{TRAIN} rows are remapped to the compact \texttt{TRAIN\_SELECTED} artifact, while \texttt{VALIDATION} rows remain canonical Phase~2 patient-shard reads. Expected divergence or non-finite loss terminates only the active range test and preserves partial LR/loss history. CUDA out-of-memory failures within a repeat trigger smaller-batch retries, and the effective batch size is reported with the screening output. The summary criterion is the median minimum smoothed loss across repeated runs. Outputs include per-model summaries, run configuration files, sampled loss settings, effective-batch records, and a LaTeX/PDF report. This stage therefore serves as a structured hyperparameter-screening process rather than a source of final claim-bearing model performance.

Runtime normalization is intentionally selected downstream rather than encoded into the Stage~4 split itself. The shared selector \RuntimeNormMethod{} chooses among \texttt{NOT\_NORMALIZED}, \texttt{REINHARD}, \texttt{RUIFROK}, \texttt{MACENKO}, and \texttt{VAHADANE}. When \texttt{VAHADANE} is selected, \RuntimeVahadaneFixedSource{} uses the Stage~4 target stain matrix as a fixed-source proxy and is therefore an approximation to canonical per-patch VAHADANE. Setting \RuntimeVahadaneExact{} restores per-patch source fitting at substantially higher runtime cost. This distinction is recorded in provenance and should be preserved in any scientific reporting of VAHADANE experiments.

\subsection{Stage 8: single-model segmentation training}

Stage~8 trains one segmentation model per execution. The training stage resolves \texttt{TRAIN} and \texttt{VALIDATION} rows from \MasterManifest, optionally respecting Stage~6 smart-sampling row state for the training split. When compact selected-TRAIN storage is enabled, the training split is remapped to the compact artifact created by Stage~6; validation remains tied to canonical Phase~2 patient shards. Before optimization, the stage verifies that training and validation patients are disjoint. Class imbalance is addressed with a \texttt{WeightedRandomSampler} based on inverse class frequencies at the patch level.
...
\begin{equation}
\mathcal{L} = \alpha\,\mathcal{L}_{\mathrm{BCE},\mathrm{fg}}
+ \beta\,\mathcal{L}_{\mathrm{Dice},\mathrm{bg}}
+ \gamma\,\mathcal{L}_{\mathrm{Dice},\mathrm{fg}}.
\end{equation}
In this expression, $\mathcal{L}$ is the total training loss, $\mathcal{L}_{\mathrm{BCE},\mathrm{fg}}$ is the foreground binary cross-entropy term, $\mathcal{L}_{\mathrm{Dice},\mathrm{bg}}$ is the Dice loss for the background channel, $\mathcal{L}_{\mathrm{Dice},\mathrm{fg}}$ is the Dice loss for the foreground channel, and $\alpha$, $\beta$, and $\gamma$ are scalar mixing weights with default values $\alpha=0.125$, $\beta=0.157$, and $\gamma=0.173$. The BCE term is applied to the foreground probability after softmax, whereas Dice terms are computed separately for the background and foreground channels. When artifact-aware loss is enabled, the per-sample loss is discounted according to the maximum filename-keyed artifact coverage stored in \MasterManifest.
...
\begin{equation}
\begin{aligned}
\mathcal{L}_{\mathrm{BCE},\mathrm{fg}} = -\frac{1}{HW}\sum_{h,w}\Bigl[
...
Online Hard Example Mining (OHEM) is available as an explicit training-only option. When enabled, OHEM ranks pixels inside each patch by error after a configured start epoch and backpropagates through only the hardest retained pixels, subject to the configured retention ratio and minimum-pixel floor. This mechanism does not alter the dataloader or class sampler, and validation keeps OHEM disabled so validation metrics remain comparable to non-OHEM runs.
...
\begin{equation}
\begin{aligned}
D_t ={}&(TP_t+FP_t)(TP_t+FN_t) \\
&\cdot (TN_t+FP_t)(TN_t+FN_t)+\varepsilon,
\end{aligned}
\end{equation}
...
\begin{multline}
\mathrm{MCC}(t) = \frac{TP_t\,TN_t-FP_t\,FN_t}{\sqrt{D_t}}.
\end{multline}
The implementation also computes the foreground prevalence at threshold $0.5$,
\[
\hat{\pi}_{0.5}=\frac{1}{N_{\mathrm{pix}}}\sum_i \indicator{p_i\geq 0.5},
\]
where $\hat{\pi}_{0.5}$ is the predicted foreground prevalence at threshold $0.5$, $N_{\mathrm{pix}}$ is the total number of evaluated pixels, $p_i$ is the predicted foreground probability for pixel $i$, and $\indicator{\cdot}$ denotes the indicator function. The implementation rejects validation epochs when $\hat{\pi}_{0.5}<0.001$ or $\hat{\pi}_{0.5}>0.99$, thereby preventing collapsed predictions from being selected as best checkpoints.
...
\]
In this expression, $R(x)\in\{0,1\}$ is the semantic region-of-interest mask at pixel $x$, $\mathrm{Down}(\cdot;s)$ denotes downsampling by scale factor $s$, $\mathrm{Up}(\cdot)$ denotes upsampling back to the original patch resolution, $\tau_{\mathrm{roi}}$ is the semantic region-of-interest threshold, and $\indicator{\cdot}$ denotes the indicator function.
...
\begin{equation}
\tau_{\mathrm{dec}}^* = \argmax_t \; \overline{\mathrm{MCC}}_{\mathrm{cal}}(t),
\end{equation}
...
\]
where $\mathcal{P}_-$ is the set of negative patients, $FP_j$ is the number of false-positive pixels for negative patient $j$, and $\indicator{FP_j=0}$ is the indicator that patient $j$ is completely free of false-positive pixels. Bootstrap confidence intervals are computed by patient-wise resampling only when at least 20 patients are available.

\section{Experimental Setup and Reproducibility}

\subsection{Software environment}

The repository targets Python~3.12 and is configured through \texttt{pyproject.toml} with \texttt{uv} as the dependency manager. Core dependencies include PyTorch, TorchVision, TorchMetrics, Albumentations, OpenSlide, torch-staintools, Shapely, scikit-learn, scikit-image, Optuna, schedulefree, Transformers, and HDF5 support through \texttt{h5py}. Development checks are defined with \texttt{pytest}, \texttt{pytest-cov}, \texttt{ruff}, and \texttt{mypy}. Environment variables are centralized in a single \texttt{.env} file derived from \texttt{.env\_example}, and stage-specific log files are written into a shared log directory. Shared runtime helpers normalize headless Matplotlib behavior in notebook environments and provide a single runtime-normalization status line for downstream stages.

\subsection{Approved model search space}

The code constrains model selection to approved registry files rather than permitting arbitrary architectures at runtime. The default registry is \RegistryNotNormalized; method-specific registries are also maintained for \texttt{MACENKO}, \texttt{REINHARD}, \texttt{RUIFROK}, and \texttt{VAHADANE}. All inspected registries use weight decay $10^{-4}$, while architecture-specific learning rates vary by normalization method. Table~\ref{tab:model-registry} summarizes the default \texttt{NOT\_NORMALIZED} registry and the current Stage~9 role assignments.
...
\midrule
Semantic & {\fontsize{6.2}{7.0}\selectfont SWIN} & \makecell[l]{tu-swin\_large\_patch4\_window7\_224\\.ms\_in22k\_ft\_in1k} & {\fontsize{6.2}{7.0}\selectfont $10^{-3}$} & {\fontsize{6.2}{7.0}\selectfont $10^{-4}$} & 8, 9, 10 \\
Semantic & {\fontsize{6.2}{7.0}\selectfont DPT} & \makecell[l]{tu-vit\_large\_patch16\_224\\.augreg\_in21k\_ft\_in1k} & {\fontsize{6.2}{7.0}\selectfont $10^{-4}$} & {\fontsize{6.2}{7.0}\selectfont $10^{-4}$} & 8, 9, 10 \\
Semantic & {\fontsize{6.2}{7.0}\selectfont SEG\-FORMER} & mit\_b5 & {\fontsize{6.2}{7.0}\selectfont $10^{-4}$} & {\fontsize{6.2}{7.0}\selectfont $10^{-4}$} & 8, 9, 10 \\
Semantic & {\fontsize{6.2}{7.0}\selectfont UPERNET} & tu-hiera\_large\_224 & {\fontsize{6.2}{7.0}\selectfont $3\times10^{-5}$} & {\fontsize{6.2}{7.0}\selectfont $10^{-4}$} & 8, 9, 10 \\
...
Spatial & {\fontsize{6.2}{7.0}\selectfont UNET++} & efficientnet-b7 & {\fontsize{6.2}{7.0}\selectfont $3\times10^{-3}$} & {\fontsize{6.2}{7.0}\selectfont $10^{-4}$} & 8, 9, 10 \\
Spatial & {\fontsize{6.2}{7.0}\selectfont FPN} & senet154 & {\fontsize{6.2}{7.0}\selectfont $10^{-4}$} & {\fontsize{6.2}{7.0}\selectfont $10^{-4}$} & 8, 9, 10 \\
Spatial & {\fontsize{6.2}{7.0}\selectfont MANET} & resnet152 & {\fontsize{6.2}{7.0}\selectfont $10^{-3}$} & {\fontsize{6.2}{7.0}\selectfont $10^{-4}$} & 8, 9, 10 \\
\bottomrule
\end{tabular}}
\end{table*}
...
\midrule
6 & Smart sampling & \makecell[l]{$n_{\mathrm{start}}=512$, $n_{\max}=15000$, growth factor $=2.0$,\\ stability threshold $=0.85$, $k_{\min}=20$, $k_{\max}=80$,\\ $m_{\max}=2000$, compact TRAIN selected enabled} & Patient-wise embedding budget, protected retention, and diversity selection \\
7 & LR screening & \makecell[l]{batch size $=32$, repeats $=3$, iterations $=100$,\\ LR range $10^{-8}$ to $10^{-1}$, Latin-hypercube samples $=12$,\\ compact TRAIN selected enabled, AMP default \texttt{fp32}} & Repeated LR-range and loss-weight screening \\
8 & Training & \makecell[l]{epochs $=100$, patience $=5$, batch size $=32$,\\ gradient accumulation $=4$, validation batch size $=96$,\\ $\alpha=0.125$, $\beta=0.157$, $\gamma=0.173$,\\ OHEM disabled by default} & Single-model optimization and checkpoint selection \\
...
The downstream stages implement reproducibility controls beyond fixed seeds alone. Training and learning-rate screening default to a \texttt{PAPER} execution profile that disables CuDNN benchmarking, enables deterministic settings where possible, and records execution metadata. Content signatures are stored for packaged HDF5 datasets, Stage~4 runtime-normalization artifacts, compact selected-TRAIN storage, and other persisted lineage sidecars. Training checkpoints carry metadata sidecars describing upstream packaging, split-bundle identity, runtime normalization, smart-sampling, compact-storage use, OHEM settings, and artifact-aware-loss lineage, and Stage~9 refuses to combine models from incompatible provenance groups. Stage~10 further validates recipe signatures and checkpoint hashes before running final inference.

The repository also applies fail-closed provenance checks at multiple dataset boundaries. Stage~2 refuses to continue when previously completed slides become stale with respect to changed extraction settings and validates the master-manifest SQLite database before manifest-backed reads. Stage~3 binds accepted-manifest filtering to immutable source-file identity, preventing stale review decisions from being silently reused after source-HDF5 regeneration. Stage~4 records source-dataset provenance, template-selection metadata, generated normalization artifacts, and split-bundle lineage in \texttt{run\_config.json} and \MasterManifest, and Stage~10 validates recipe and checkpoint compatibility before test-time execution. These mechanisms are methodologically important because they reduce the risk of reporting results from silently mixed experimental lineages.
...
Two implementation choices also require careful wording in any claim-bearing report. First, the default \texttt{fixed\_source} VAHADANE backend is a pragmatic fixed-matrix approximation and should not be described as exact per-patch VAHADANE. Second, the optional large-pool GIST selector in Stage~6 uses landmark preselection before the facility-location step and therefore preserves a GIST-style selection objective without preserving the full paper-exact search space or theoretical guarantee.
...
\textbf{Work in progress.} A discussion section can be completed safely only after empirical results and cohort details are available. Based on the code alone, the main methodological strengths are the HDF5-native downstream workflow, the train-only runtime-normalization artifact policy, deterministic patch identifiers, compact selected-TRAIN storage for smart-sampled runs, and provenance-aware fail-closed checks. Any claims regarding clinical utility, robustness, or superiority over baselines require external evidence and are intentionally omitted.

\section{Conclusion}

The implemented repository defines a reproducibility-oriented computational pathology workflow spanning slide-level artifact characterization, HDF5-native patch extraction, manifest-based graph cleaning, canonical source-dataset packaging, patient-identifier-aware splitting, smart-sampled training-data filtering with optional compact storage, single-model optimization, validation-only ensemble design, and frozen test-time inference. The code therefore supports a manuscript-grade description of methodology and execution logic. Final scientific conclusions, however, must still be completed from experiment artifacts rather than inferred from source code alone.

\appendix

\section{Code-grounded placeholders requiring experiment artifacts}

The following items were not verifiable from source inspection alone and should be populated before submission: dataset provenance and cohort counts; scanner or site distribution; hardware used for the final reported runs; whether artifact-aware loss, OHEM, smart sampling, compact selected-TRAIN storage, and runtime normalization were enabled in the claim-bearing experiment; which VAHADANE backend was used if applicable; exact checkpoint identities chosen for the ensemble; final region-of-interest and decision thresholds; and all quantitative results, plots, and statistical comparisons.

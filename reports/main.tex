...
Stage~6 optionally reduces the effective training set while preserving manifest-backed provenance and validation/test semantics. The stage reads \texttt{TRAIN} rows from \texttt{master\_manifest.sqlite}, loads pixels from canonical Phase~2 patient shards, builds a patient-wise patch index, and extracts Phikon-v2 embeddings. Positive-label rows and rows with positive mask pixels are protected from reduction by default, so the diversity selector operates only on reducible candidates while protected rows remain eligible for downstream training. This policy makes the storage optimization explicit: selection changes the training-row subset, but it does not rewrite validation or test rows.
...
\]
Here, $z_i$ is the embedding for patch $i$ and corresponds to the \texttt{last\_hidden\_state[:,0,:]} tensor returned by the model. For each patient, the stage starts from an initial candidate count $n_0$ and iteratively increases the embedding budget according to
\[
...
Runtime normalization is intentionally selected downstream rather than encoded into the Stage~4 split itself. The shared selector \texttt{RUNTIME\_NORMALIZATION\_METHOD} chooses among \texttt{NOT\_NORMALIZED}, \texttt{REINHARD}, \texttt{RUIFROK}, \texttt{MACENKO}, and \texttt{VAHADANE}. When \texttt{VAHADANE} is selected, \texttt{RUNTIME\_VAHADANE\_BACKEND=fixed\_source} uses the Stage~4 target stain matrix as a fixed-source proxy and is therefore an approximation to canonical per-patch VAHADANE. Setting \texttt{RUNTIME\_VAHADANE\_BACKEND=torch\_staintools\_exact} restores per-patch source fitting at substantially higher runtime cost. This distinction is recorded in provenance and should be preserved in any scientific reporting of VAHADANE experiments.

\subsection{Stage 8: single-model segmentation training}

Stage~8 trains one segmentation model per execution. The training stage resolves \texttt{TRAIN} and \texttt{VALIDATION} rows from \texttt{master\_manifest.sqlite}, optionally respecting Stage~6 smart-sampling row state for the training split. When compact selected-TRAIN storage is enabled, the training split is remapped to the compact artifact created by Stage~6; validation remains tied to canonical Phase~2 patient shards. Before optimization, the stage verifies that training and validation patients are disjoint. Class imbalance is addressed with a \texttt{WeightedRandomSampler} based on inverse class frequencies at the patch level.
...
The code constrains model selection to approved registry files rather than permitting arbitrary architectures at runtime. The default registry is \texttt{training\_model\_registry\_NOT\_NORMALIZED.json}; method-specific registries are also maintained for \texttt{MACENKO}, \texttt{REINHARD}, \texttt{RUIFROK}, and \texttt{VAHADANE}. All inspected registries use weight decay $10^{-4}$, while architecture-specific learning rates vary by normalization method. Table~\ref{tab:model-registry} summarizes the default \texttt{NOT\_NORMALIZED} registry and the current Stage~9 role assignments.

...
Stage~6 optionally reduces the effective training set to a more compact subset while preserving within-patient diversity. The stage reads \texttt{TRAIN} rows from \texttt{master\_manifest.sqlite}, loads pixels from canonical Phase~2 patient shards, builds a patient-wise patch index, and extracts encoder embeddings using the encoder of a segmentation model initialized with ImageNet weights. Global-average pooled feature maps are used as patch descriptors. For each patient, the algorithm begins with a starting sample size and estimates clustering stability through the adjusted Rand index between two random subsamples. The number of embedded patches is then increased multiplicatively until the stability threshold is reached or configured limits are exceeded.
...
Once the patch budget for a patient is determined, diversity selection is performed with \texttt{MiniBatchKMeans}. Patches are then sampled approximately uniformly across clusters, and the implementation persists only sidecar summaries plus SQLite row-state updates such as \texttt{sampling\_decision} and \texttt{is\_stage7\_selected}. Downstream training code therefore continues to load canonical Phase~2 patient shards directly while resolving the selected training rows from the master manifest.
...
Stage~8 trains one segmentation model per execution. The training stage resolves \texttt{TRAIN} and \texttt{VALIDATION} rows from \texttt{master\_manifest.sqlite}, optionally respecting Stage~6 smart-sampling row state for the training split, and then loads pixels directly from canonical Phase~2 patient shards. Before optimization, it verifies that training and validation patients are disjoint. Class imbalance is addressed with a \texttt{WeightedRandomSampler} based on inverse class frequencies at the patch level.
...
The downstream stages implement reproducibility controls beyond fixed seeds alone. Training and learning-rate screening default to a \texttt{PAPER} execution profile that disables CuDNN benchmarking, enables deterministic settings where possible, and records execution metadata. Content signatures are stored for packaged HDF5 datasets, Stage~4 runtime-normalization artifacts, and other persisted lineage sidecars. Training checkpoints carry metadata sidecars describing upstream packaging, split-bundle identity, runtime normalization, smart-sampling, and artifact-aware-loss lineage, and Stage~9 refuses to combine models from incompatible provenance groups. Stage~10 further validates recipe signatures and checkpoint hashes before running final inference.

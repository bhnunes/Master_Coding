...
class H5MetadataIndex:
    ...
    def _build_index(self):
        ...
        print(
            f"[Data] Indexed {len(self.patient_map)} patients, {self.total_samples} total patches."
        )
...
def calculate_stability_score(emb_s1, emb_s2, idx_s1, idx_s2, config):
    ...
    mbk1 = MiniBatchKMeans(
        n_clusters=k, batch_size=512, n_init=3, random_state=config.SEED, max_iter=100
    ).fit(emb_s1)
    mbk2 = MiniBatchKMeans(
        n_clusters=k, batch_size=512, n_init=3, random_state=config.SEED + 1, max_iter=100
    ).fit(emb_s2)
...
def select_diverse_samples(embeddings, global_indices, m_target, config):
    ...
    clusterer = MiniBatchKMeans(
        n_clusters=k, batch_size=1024, n_init=3, random_state=config.SEED
    ).fit(embeddings)
...
def guardrail(f_src):
    ...
    if missing:
        raise KeyError(
            f"Source HDF5 missing required keys: {missing}. Available: {list(f_src.keys())}"
        )


def resolve_filename_key(f_src):
    if "filenames" in f_src:
        return "filenames"
    if "filename" in f_src:
        return "filename"
    raise KeyError(f"Source HDF5 missing filename dataset. Available: {list(f_src.keys())}")
...
def write_filtered_hdf5(config, selected_indices):
    ...
    with h5py.File(src_path, "r") as f_src, h5py.File(dst_path, "w") as f_dst:
        ...
        guardrail(f_src)
        filename_key = resolve_filename_key(f_src)
        ...
        for key in ["images", "masks", "patient_ids", "labels", filename_key]:
            ...
            if key in f_src:
                ...
                ds = f_dst.create_dataset(
                    target_key, shape=tuple(shape), dtype=f_src[key].dtype, **kw
                )
        ...
        for i in tqdm(range(0, total, batch_size), desc="Writing HDF5"):
            ...
            for key in ["images", "masks", "patient_ids", "labels", filename_key]:
                ...
                if key in f_src:
                    ...
                    f_dst[target_key][i : i + len(batch_indices)] = data

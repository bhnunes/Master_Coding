...
- Important remaining scientific risks after the remediation pass, and the best starting points for the next session, are:
  ...
  - Highest remaining Stage 12 risk is ambiguous threshold semantics if one threshold still controls both semantic ROI gating and final hard decisions. Preferred direction is separate `roi_gate_threshold` and `decision_threshold`, with the decision threshold tuned on a Stage 11 internal calibration subset using `MCC`. Start in `helpers/ensemble_optimizer/optimization.py`, `helpers/ensemble_optimizer/reporting.py`, `helpers/ensemble_inference/recipe.py`, `helpers/ensemble_inference/pipeline.py`, and `helpers/ensemble_inference/inference.py`.
  - High remaining reproducibility risk: stale upstream artifacts may still be reused invisibly when paths remain stable but contents change. Strengthen content-based provenance checks across Stage 7 packaging, Stage 8 smart sampling, Stage 11 recipe generation, and Stage 12 inference handoff. Start in `helpers/packaging/writer.py`, `helpers/smart_sampling/writer.py`, `helpers/ensemble_optimizer/reporting.py`, and `helpers/ensemble_inference/pipeline.py`.

...
- Current validation status as of 2026-03-21 after the Stage 5 / Stage 11 / Stage 12 threshold-semantics redesign: targeted touched tests pass (`59 passed`), `uv run ruff check .` passes, and `uv run mypy .` passes. The earlier full-suite validation baseline from the prior remediation pass was `uv run pytest` passing (`362 passed`). The earlier full-coverage baseline before this remediation wave was `uv run pytest --cov=helpers --cov-report=term-missing` passing with overall `helpers` coverage at `72%`; coverage should be re-measured again before the next coverage-first session.

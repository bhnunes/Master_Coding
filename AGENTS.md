...
- The current manuscript frontmatter in `reports/main.tex` is initialized with the working title `Framework end to end with CNNs and Transformers for detection of cancer in digital pathology`, author `Bruno Henrique Nunes`, and email `bruno.nunes.1987@gmail.com`. Treat these as the current draft values unless the user asks to change them.
...
- The manuscript has already been strengthened beyond prose-only description: `reports/main.tex` now includes code-grounded equations for Stage 4.1 grouped sampling, Stage 4.2 contamination and threshold tuning, Stage 8 smart sampling, Stage 10 BCE+Dice training loss and MCC sweep, Stage 11 two-stream spatial gating and optimization objectives, and Stage 12 evaluation metrics, plus tables for the approved model registry and core optimization settings.
- When editing the manuscript, preserve double-column layout safety. Prefer multiline equations, compact notation, and table-specific font adjustments over wide single-line formulas. Rebuild `reports/main.pdf` after manuscript edits and inspect for overflow/overfull issues, especially around equations and tables.

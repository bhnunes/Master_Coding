...
Use the current environment's image-generation tool or skill to create `./outputs/figure.png`, then include it with `\includegraphics[width=0.6\textwidth]{figure.png}` in a `figure` environment. Request "white background, clean, no text" for best results. See [advanced-features.md](advanced-features.md).

...
| **21+ pages** | Batch-7 pipeline | ceil(N/7) | Each worker gets about 7 pages (0-error sweet spot). Process page ranges in parallel when the environment supports multiple workers. |
...
Two workers, each with about half the pages. Run them in parallel when the environment supports it.

### For batch-7 pipeline (21+ pages):

Launch `ceil(N/7)` workers, or process those ranges sequentially if parallel execution is unavailable.

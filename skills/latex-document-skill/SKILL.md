...
description: >
  Create, compile, validate, and convert LaTeX/PDF documents with bundled
  templates, scripts, references, and previews. Use when Codex needs to work on
  resumes, CVs, cover letters, reports, invoices, academic papers, theses,
  Beamer slides, posters, exams, lecture notes, books, cheat sheets, fillable
  forms, mail merge documents, charts, tables, diagrams, citations, BibTeX or
  biblatex, multi-language LaTeX, Markdown/DOCX/HTML to or from LaTeX
  conversion, PDF utilities such as merge/encrypt/optimize/extract pages,
  LaTeX linting, word counts, package checks, or PDF/OCR-to-LaTeX reconstruction
  from printed or handwritten material. Prefer the skill's scripts for
  repeatable compilation, validation, charts, PDF operations, and format
  conversion; load detailed reference files only for the requested document type
  or operation.
---
...
- On Windows, run `.sh` helper scripts with Git Bash when available, e.g. `"C:\Program Files\Git\bin\bash.exe" <skill_path>/scripts/compile_latex.sh ...`. The default `bash.exe` may point to disabled WSL.
- Python helper scripts require a real Python installation on PATH. If `python` resolves to the Windows Store alias or `py` points to a missing interpreter, use `uv run --with-requirements <skill_path>/requirements.txt python <skill_path>/scripts/<script>.py ...` when `uv` is available, or avoid Python helpers until Python is repaired or installed.
- The core LaTeX workflow can still use MiKTeX executables directly (`pdflatex`, `xelatex`, `lualatex`, `latexmk`) when shell helpers are unavailable.
...
5. **Ask the user which enrichment elements they want** when the request leaves that open. Offer relevant options based on document type:
   - **AI-generated images** -- custom illustrations, diagrams, photos (use the `imagegen` skill/tool when available)
...
8. Generate external assets based on user's element choices:
   - AI images: use the current environment's image-generation tool or skill, then save the result as a normal image asset for LaTeX
...
**Scaling strategy**: 1-10 pages = single pass; 11-20 pages = split in half; 21+ pages = batch-7 pipeline with parallelizable page ranges when the environment supports multiple workers.
...
| IEEE journal, IEEE Transactions, IEEE two-column paper | `assets/templates/ieee-twocolumn.tex` + [ieee-journal-twocolumn-guide.md](references/ieee-journal-twocolumn-guide.md) | `IEEEtran` |
| Book | `book.tex` | `book` |

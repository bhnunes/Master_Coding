...
count_words() {
    if command -v detex &>/dev/null; then
        detex "$TEX_FILE" 2>/dev/null | wc -w | tr -d ' '
        return 0
    fi
    if command -v texcount &>/dev/null; then
        texcount -sum -1 "$TEX_FILE" 2>/dev/null | tail -n 1 | tr -d ' '
        return 0
    fi
    echo "Error: neither detex nor texcount was found" >&2
    echo "Install detex/texlive-extra-utils or ensure MiKTeX texcount can run." >&2
    exit 1
}

plain_text_char_count() {
    if command -v detex &>/dev/null; then
        detex "$TEX_FILE" 2>/dev/null | wc -c | tr -d ' '
        return 0
    fi
    sed -E 's/\\[a-zA-Z]+\*?(\[[^]]*\])?(\{[^}]*\})?//g; s/[{}$]//g' "$TEX_FILE" \
        | wc -c | tr -d ' '
}

# Count words using detex, or texcount when detex is unavailable
...
if [[ "$DETAILED" == true ]]; then
    ...
    CHAR_COUNT=$(plain_text_char_count)
    echo ""

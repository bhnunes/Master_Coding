...
detect_pkg_manager() {
  ...
  if command -v apt-get &>/dev/null; then
    ...
    _detected_pkg_manager="winget"
  elif command -v choco &>/dev/null; then
    _detected_pkg_manager="choco"
  else
...
run_pkg_install() {
  ...
  case "$mgr" in
    ...
    pacman)
      ...
      ;;
    winget)
      echo ":: Installing (winget): ${packages[*]}" >&2
      for pkg in "${packages[@]}"; do
        case "$pkg" in
          graphviz) winget install --id Graphviz.Graphviz --accept-source-agreements --accept-package-agreements --silent --disable-interactivity >&2 2>&1 ;;
          qpdf) winget install --id QPDF.QPDF --accept-source-agreements --accept-package-agreements --silent --disable-interactivity >&2 2>&1 ;;
          *) winget install --id "$pkg" --accept-source-agreements --accept-package-agreements --silent --disable-interactivity >&2 2>&1 ;;
        esac
      done
      ;;
    choco)
      echo ":: Installing (choco): ${packages[*]}" >&2
      choco install -y "${packages[@]}" >&2 2>&1
      ;;
    *)
...
# --- qpdf availability helper ---
ensure_qpdf_available() {
  ...
  mgr="$(detect_pkg_manager)"
  case "$mgr" in
    apt|brew|dnf|apk|pacman|winget|choco)
      run_pkg_install qpdf
      ;;
    *)
      ...
      echo "  Windows:        winget install --id QPDF.QPDF" >&2
      return 1
      ;;
  esac
  ...
  echo "Error: qpdf installation failed or qpdf is not on PATH" >&2
  return 1
}
...
ensure_graphviz_available() {
  ...
  case "$(uname -s 2>/dev/null || echo unknown)" in
    MINGW*|MSYS*|CYGWIN*)
      if [[ -x /c/Program\ Files/Graphviz/bin/dot.exe ]]; then
  ...
  case "$mgr" in
    ...
    *)
      ...
      echo "  Windows:        winget install --id Graphviz.Graphviz" >&2
      return 1
      ;;
  esac
  ...
  echo "Error: Graphviz installation failed or dot is not on PATH" >&2
  return 1
}

# MiKTeX wrappers such as latexdiff.exe and latexmk.exe need a full Perl. Git
# Bash ships a minimal Perl first in PATH, so prefer Strawberry Perl when present.
prefer_strawberry_perl_on_windows() {
  case "$(uname -s 2>/dev/null || echo unknown)" in
    MINGW*|MSYS*|CYGWIN*)

...
source "${SCRIPTS_DIR}/install_deps.sh"
prefer_strawberry_perl_on_windows
ensure_graphviz_available >/dev/null 2>&1 || true

has_word_counter() {
    command -v detex &>/dev/null || command -v texcount &>/dev/null
}
...
test_latex_analyze_basic() {
    ...
    if ! has_word_counter; then
        skip "latex_analyze.sh - Basic execution" "detex/texcount not available"
        return
    fi
...
test_latex_analyze_word_count() {
    ...
    if ! has_word_counter; then
        skip "latex_analyze.sh - Word count detection" "detex/texcount not available"
        return
    fi
...
test_latex_analyze_figures() {
    ...
    if ! has_word_counter; then
        skip "latex_analyze.sh - Figure counting" "detex/texcount not available"
        return
    fi
...
test_latex_analyze_tables() {
    ...
    if ! has_word_counter; then
        skip "latex_analyze.sh - Table counting" "detex/texcount not available"
        return
    fi
...
test_latex_analyze_equations() {
    ...
    if ! has_word_counter; then
        skip "latex_analyze.sh - Equation counting" "detex/texcount not available"
        return
    fi
...
test_latex_analyze_sections() {
    ...
    if ! has_word_counter; then
        skip "latex_analyze.sh - Section detection" "detex/texcount not available"
        return
    fi
...
test_latex_analyze_todo_detection() {
    ...
    if ! has_word_counter; then
        skip "latex_analyze.sh - TODO detection" "detex/texcount not available"
        return
    fi
...
test_latex_analyze_unreferenced_labels() {
    ...
    if ! has_word_counter; then
        skip "latex_analyze.sh - Unreferenced label detection" "detex/texcount not available"
        return
    fi
...
test_latex_analyze_missing_labels() {
    ...
    if ! has_word_counter; then
        skip "latex_analyze.sh - Missing label detection" "detex/texcount not available"
        return
    fi

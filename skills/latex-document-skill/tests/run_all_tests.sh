...
run_test_suite() {
  ...
  if [[ "$test_script" == *.py ]]; then
    if command -v uv &>/dev/null; then
      if (cd "$SKILL_DIR" && uv run --with-requirements requirements.txt --with pytest --with pypdf --with reportlab python -m pytest "$test_script" -q); then
        exit_code=0
      else
        exit_code=$?
      fi
    elif command -v python &>/dev/null; then
      if python -m pytest "$test_script" -q; then
        exit_code=0
      else
        exit_code=$?
      fi
    else
      echo -e "${YELLOW}[SKIP]${NC} $suite_name - python/uv not found"
      exit_code=125
    fi
  elif bash "$test_script"; then
    exit_code=0
  else
    exit_code=$?
  fi
  ...
  local duration
  duration=$(awk -v start="$start_time" -v end="$end_time" 'BEGIN { printf "%.2f", end - start }')
  ...
  if [[ $exit_code -eq 0 ]]; then
    ...
    echo -e "${YELLOW}SKIPPED${NC} - $suite_name (${duration}s)"
    SUITE_RESULTS+=("SKIP")
    SKIPPED_SUITES=$((SKIPPED_SUITES + 1))
  else
...
print_summary() {
  ...
  for duration in "${SUITE_DURATIONS[@]}"; do
    total_time=$(awk -v total="$total_time" -v duration="$duration" 'BEGIN { printf "%.2f", total + duration }')
  done
...
main() {
  ...
  run_test_suite "${SCRIPT_DIR}/test_pdf_forms.py" "PDF Form Tests"

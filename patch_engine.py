...
# **MODIFIED**: This is a pure engine. All configuration is passed in.
...
def polygons_to_mask(mask_shape, polygons_level0, scale_factor, patch_coords):
    ...
    for poly_l0 in polygons_level0:
        ...
        try:
            ...
            if not anno_poly_l0.is_valid: anno_poly_l0 = anno_poly_l0.buffer(0)
        except Exception: continue
        if not prep_win.intersects(anno_poly_l0): continue
        try:
            intersection = win_poly_l0.intersection(anno_poly_l0)
        except shapely.errors.TopologicalError: continue
...
def process_window(args):
    """Generic window processor. Relies only on its arguments."""
...
def run_extraction(handler: BaseHandler, path_Image: str, **kwargs):
    ...
    try:
        ...
        for geom in scaled_polys_raw:
            if geom.geom_type == 'Polygon': scaled_polys_flat.append(geom)
            elif geom.geom_type == 'MultiPolygon': scaled_polys_flat.extend(list(geom.geoms))
        ...
        errors = [msg for status, msg in results if status is False]
        
        if errors:
            raise Exception("Errors occurred in workers. See log for details.")

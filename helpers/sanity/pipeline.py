...
def run_sanity_pipeline(config: SanityConfig) -> SanityReport:
    ...
    manifest_schema_result = check_manifest_schema(manifest_df)
    dataset_checks = {
        "Manifest Schema": manifest_schema_result,
    }
    if manifest_schema_result.status == "FAIL":
        return SanityReport(
